...
This library adopts [Unknown v0.3](Unknown-LICENSE-v0.3.md): public domain with
...
| [`yau-calabi-yau-fifty-years-reading-note-v0.md`](yau-calabi-yau-fifty-years-reading-note-v0.md), [`…-v1.md`](yau-calabi-yau-fifty-years-reading-note-v1.md) | original Chinese study note on canonical metrics, stability, mirror symmetry and higher-genus functions, with 23 source groups and an admission record; v1 appends two further bibliography groups on the six-lines-to-K3 literature and records that this literature runs on six **lines**, not on six points on a conic | proposed document; catalog home `geometry`, lineage `external-reference` |
...
| `yang-lu-inequality-proving-external-reference-v0.md` | one external-reference registration over Yang Lu's automated inequality proving and machine discovery, with the depth of each reading recorded | proposed document; catalog home `geometry`, lineage `external-reference` |
